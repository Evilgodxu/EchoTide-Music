...
- **多平台在线搜索**:聚合网易云、QQ 音乐、酷狗、酷我、咪咕五平台。内置音源只提供**基础的歌曲搜索、封面获取与歌词获取**——播放并非其承诺的能力,应用仅在音源恰好返回可播放直链时,理论上支持试听与免费歌曲。此类曲目仍可使用搜索历史与音质选择(无损 / 高品 / 标准),并尝试缓存到本地(缓存完成后改为本地播放);无损档内先取 Hi-Res,取不到才降级到普通无损,故选择弹窗不额外暴露档位。平台切换菜单同时列出已启用代理音源声明的**自定义平台**——应用未内置的平台正是靠它获得搜索、播放、歌词与封面能力(见「代理音源」)
...
- **歌单导入**:粘贴平台歌单分享链接,解析预览曲目后尝试整单缓存到本地并建为自定义歌单(网易云内置解析,其余平台需代理音源)
- **同步歌词**:滚动歌词 + 逐字级时间轴(可开关),支持在线歌词匹配 / 刷新、本地歌词导入、内嵌歌词与歌词原文编辑(校验时间戳前缀),并可对歌词时间进行微调;行级歌词可在应用内对齐为逐字时间轴(对解码音频做单调 DP 对齐,支持后台继续);在歌词区上下拖拽可实时调整播放进度——纵向位移占优即由歌词接管手势,横向拖拽则完整让给面板滑动;接管后按滑动速度分流:慢速拖拽才由歌词跟随手指,与某句对齐释放即从该句起播并让基准标识转为确认色、未对齐则回弹原处;一甩即走的快速滑动全程不带动歌词,直接按上下滑动切歌处理。音乐面板上下边缘改用逐行降低透明度,取代渐隐蒙层,紧邻边缘的一行仍可辨读
...
- **曲库分析**:按格式定位曲库,检测音质异常(频谱分析)与疑似 AI 生成音乐,并归类在线曲目,可随时重新分析。结论来自频谱页全曲分析的曲目会被锁定,曲库分析的分段采样不再改写它——歌单过滤与曲库统计因此口径一致;缓存到本地或无损升级改变音频后解除锁定,重新参与分析
...
- **无损音质升级**:为当前曲目匹配 Hi-Res 或普通无损在线音源(优先 Hi-Res,不降级到有损档)并就地替换播放源。该项依赖**代理音源**——内置音源不返回无损播放直链,故功能仅对已启用代理音源覆盖的平台可用
...
在线音乐搜索依赖第三方公共网络接口(网易云 / QQ 音乐 / 酷狗 / 酷我 / 咪咕)。内置音源仅用于基础的歌曲搜索、封面获取与歌词获取,不承诺播放能力,应用仅在音源返回可播放直链时理论上支持试听与免费歌曲;无损升级依赖代理音源。接口可用性随地区与歌曲而异。应用仅供个人学习交流使用,请支持正版版权方。
