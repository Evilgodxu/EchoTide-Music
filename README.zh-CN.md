...
- **同步歌词**:滚动歌词 + 逐字级时间轴(可开关),支持在线歌词匹配 / 刷新、本地歌词导入、内嵌歌词与歌词原文编辑(校验时间戳前缀),并可对歌词时间进行微调;行级歌词可在应用内对齐为逐字时间轴(对解码音频做单调 DP 对齐,支持后台继续);在歌词区上下拖拽可实时调整播放进度——纵向位移占优即由歌词接管手势,横向拖拽则完整让给面板滑动;接管后的滑动再按离手速度分流,慢速拖拽与某句对齐释放即从该句起播并让基准标识转为确认色、未对齐则回弹原处,一甩即走的快速滑动不跳转歌词进度而按上下滑动切歌处理。音乐面板上下边缘改用逐行降低透明度,取代渐隐蒙层,紧邻边缘的一行仍可辨读
