...
**忆潮音乐(Echo Tide)** 是一款基于 Jetpack Compose 构建的全功能 Android 音乐播放器。除常规应用内播放外,它还提供可悬浮于任意应用之上的**悬浮音乐面板**与**迷你播放器**——无论正在游戏、浏览器还是其他界面,音乐始终触手可及。此外还提供基于歌词画像的**每日推荐**轮播与五平台聚合搜索。
...
- **同步歌词**:滚动歌词 + 逐字级时间轴(可开关),支持在线歌词匹配 / 刷新、本地歌词导入、内嵌歌词与歌词原文编辑(校验时间戳前缀),并可对歌词时间进行微调;行级歌词可在应用内对齐为逐字时间轴(对解码音频做单调 DP 对齐,支持后台继续);在歌词区上下拖拽可实时调整播放进度(与某句对齐释放即从该句起播,未对齐则回弹原处)
...
- **曲库分析**:按格式定位曲库,检测音质异常(频谱分析)与疑似 AI 生成音乐,并归类在线曲目,可随时重新分析
...
- **存储管理**:独立页面统计图片缓存、中转文件、异常日志、歌词缓存、歌曲缓存、更新安装包、曲库分析缓存与用户偏好等占用,按「可清理 / 应用私有 / 用户数据」分组展示,支持下拉重新采样,并可一键清理应用缓存、异常日志与已下载的更新安装包

## 页面

| 页面 | 内容 |
| --- | --- |
| 首页 | 权限引导对话框(全部授权后自动关闭)、沉浸式播放器(旋转碟片封面 + 封面取色渐变背景)、同步歌词(字号与行数可调,可上下拖拽调整播放进度,支持应用内逐字对齐)、可刷新、可搜索并支持排序与分享链接导入的播放列表、收藏、定时关闭、无损升级、曲库分析、横屏模式(含 3D 封面轮播)、右滑在线搜索(五平台切换 + 音质选择,另含每日推荐轮播)与左滑歌单面板、上下滑动切歌、点击歌手信息行直达歌手歌单(长按封面 / 标题可刷新封面、歌词,编辑歌词及重命名) |
| 设置 | 外观(主题)、语言、播放(悬浮播放 / 逐字渲染 / 滑动切歌)与排版入口、黑名单(数量展示与重置)、存储(进入存储管理页)、代理音源(导入 / 启停 / 移除第三方音源)、关于(版本、检查更新、分享今日日志、GitHub 链接、QQ 群) |
...
│       │   │   │   ├── analysis/        #     无损格式、音质异常与 AI 音乐分析、音频信息读取、逐字歌词对齐
│       │   │   │   ├── blacklist/       #     黑名单存储
│       │   │   │   ├── clip/            #     分享、默认铃声 / 闹钟设置、可读 URI 解析
│       │   │   │   ├── download/        #     在线曲目下载与缓存
